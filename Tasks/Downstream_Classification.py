...
class DS_Classification_Trainer(object):
    def __init__(self, args: argparse, check_path: str):
        ...
        # Define Model
        # self.trained_rep = torch.load(f'{self.check_path}/best_representation.pt_best.pt')
    ...
    def test(self):
        ...
        with tqdm.tqdm(**get_tqdm_config(total=steps_per_epoch, leave=False, color='blue')) as pbar:
            with torch.no_grad():
                ...
                # precision, recall, _ = precision_recall_curve(targets_onehot, probs, pos_label=1)
                #
