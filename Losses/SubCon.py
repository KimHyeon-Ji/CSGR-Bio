...
class Sub_SupCon(nn.Module):
    ...
    def forward(self, X_data, subject=None, labels=None, mask=None): # shape: (256, 32)
        ...
        same_subject_mask = torch.eq(subject, torch.tensor(subject_list).to(self.device)).float() # 같은 피험자 1
        ...
        logit_mask = torch.index_select(logit_mask, dim=0, index=not_zero_idx[0])
