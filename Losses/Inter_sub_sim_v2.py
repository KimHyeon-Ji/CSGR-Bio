...
class Inter_sub_sim_v2(nn.Module):
    def __init__(self, args: argparse):
        super(Inter_sub_sim_v2, self).__init__()
    ...
    def forward(self, X_data, subject=None, labels=None, mask=None):
        ...
        # logit mask
        same_subject_mask = torch.eq(subject, torch.tensor(subject_list).to(self.device)).float() 
        same_subject_mask = same_subject_mask.repeat(contrast_count, 1)
        logit_mask = same_subject_mask * labels_mask  
