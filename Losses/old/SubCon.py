...
class Sub_SupCon(nn.Module):
    def __init__(self, args: argparse):
        super(Sub_SupCon, self).__init__()
    ...
    def forward(self, X_data, subject=None, labels=None, mask=None): # shape: (256, 32)
        ...
        # logit mask: 같은 클래스이면서 같은 피험자
        same_subject_mask = torch.eq(subject, torch.FloatTensor(np.array(subject_list))).float().to(self.device)  # 같은 피험자 1
        same_subject_mask = same_subject_mask.repeat(contrast_count, 1)
        logit_mask = same_subject_mask * labels_mask  # 같은 클래스이면서 같은 피험자 -> 1
        logit_mask = torch.where(logit_mask == 1, 0, 1)
        logit_mask = logit_mask.detach().cpu().numpy()
        logit_mask = torch.FloatTensor(np.delete(logit_mask, zero_pos_idx, axis=0)).float().to(self.device)

        sim = sim * logit_mask
        loss_labels = mask

        loss = self.criterion(sim, loss_labels) / mask.sum(1)
