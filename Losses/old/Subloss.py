...
class SubLoss(nn.Module):
    ...
    def forward(self, X_data, subject=None, mask=None): # shape: (256, 32)
        ...
        logits_cen_mask = torch.ones_like(max_masked_anchor_dot_contrast)  # torch.Size([256, 1])
        logits_mask = torch.concat([logits_ins_mask, logits_cen_mask], dim=1)

        ## 전체 mask 만들기 -> positive, negative 구분
        ins_mask = torch.zeros_like(masked_anchor_dot_contrast_ins)  # torch.Size([256, 256])
        cen_mask = torch.ones_like(max_masked_anchor_dot_contrast)  # torch.Size([256, 1])
        total_mask = torch.concat([ins_mask, cen_mask], dim=1)  # torch.Size([256, 257])

        # torch.Size([256, 257])
        total_logits = torch.concat([masked_anchor_dot_contrast_ins, max_masked_anchor_dot_contrast], dim=1)

        total_logits_max, _ = torch.max(total_logits, dim=1, keepdim=True)  # torch.Size([256, 1])
        total_logits = total_logits - total_logits_max.detach()  # torch.Size([256, 257])

        # compute log_prob
        exp_total_logits = torch.exp(total_logits) * logits_mask  # torch.Size([256, 257])
        log_prob = exp_total_logits - torch.log(exp_total_logits.sum(1, keepdim=True)) # torch.Size([256, 257])


        # compute mean of log-likelihood over positive
        mean_log_prob_pos = (total_mask * log_prob).sum(1) / total_mask.sum(1)  # torch.Size([256])

        # loss
        loss = - (self.temperature / self.base_temperature) * mean_log_prob_pos  # loss.shape: (256,)
        loss = loss.view(anchor_count_ins, batch_size).mean()
