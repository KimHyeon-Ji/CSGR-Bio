...
from Dataloaders.Dataset import mit_bih_afib, UCI_HAR, UniMiB, sleep_edf, WISDM, MyoArmband, BCI_motor_imagery, MNE_alphawaves, Auditory_EEG, EEG_mental
from Utils.Path import mit_bih_afib_path, UCI_HAR_path, UniMiB_path, sleep_edf_path, \
...
def make_dataloader(args: argparse, **kwargs: dict):
    ...
    if args.dataset == 'MyoArmband':
        ...
        # x_train = np.load(os.path.join(data_path, f'x_train_{args.dataset_ver}_ab{args.abl_ver}.npy'))
        # x_aug_train = np.load(os.path.join(data_path, f'x_aug_train_{args.dataset_ver}_ab{args.abl_ver}.npy'))
        # y_train = np.load(os.path.join(data_path, f'y_train_{args.dataset_ver}_ab{args.abl_ver}.npy'))
        # s_train = np.load(os.path.join(data_path, f's_train_{args.dataset_ver}_ab{args.abl_ver}.npy'))
        #
        ...
        dataset = sleep_edf.sleep_edf_Dataset(args, x, x_aug, y, s, mode='random')
        testset = sleep_edf.sleep_edf_Dataset(args, x, x_aug, y, s, mode='test')
